			...
			<?php foreach($filtroOrdenesDeCompra as $filtroOrdenesDeCompras) : ?>
				...
				<td>
					<?php
							...
							{
								echo '<span class="label label-default tool" data-toggle="tooltip" data-placement="top" title="'.$ordenCompras["ComprasProductosTotale"]["ComprasProductosRechazo"][0]["motivo"]. '"> Rechazado G</span>';
							}
				...
				<td>
					...
					<?php //if($filtroOrdenesDeCompras["estado"] == 1 || $filtroOrdenesDeCompras["estado"] == 2 || $filtroOrdenesDeCompras["estado"] == 4) :?>
						<a class="btn-sm btn btn-warning tool btn-xs" title="" data-placement="top" data-toggle="tooltip" href="<?php echo $this->Html->url(array("action"=>$plantilla, $filtroOrdenesDeCompras["idOrdenCompra"])); ?>" data-original-title="Usar plantilla"><i class="fa fa-recycle"></i></a/>
					<?php //endif; ?>
