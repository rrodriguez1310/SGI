<form class="form-horizontal" method="post" id="fileinfo" name="fileinfo" role="form">
	<?php
		...
		$attributes = array('div' => 'input', 'type' => 'radio', 'options' => $options, 'class'=>'requerido radio_botom');
	...
	<fieldset>
		...
		<div class="form-group">
			...
			<div class="col-md-6">
				<?php echo $this->Form->input('fecha_documento', array("class"=>"col-xs-4 form-control requerido", "label"=>false, 'placeholder'=>'Fecha Documento', 'default'=>''));?>
			</div>
		</div>
	...
	<?php //echo $this->Form->input('id_clonado', array('default'=>$id));?>
	<?php //echo $this->Form->input('clonar', array('default'=>1));?>
	...
	<div class="form-group">
		<div class="col-md-6 col-md-offset-4">
			<button type="submit" class="btn-block btn btn-primary btn-lg generarOrden"><i class="fa fa-file-text-o"></i> Generar Documento Tributario</button>
		</div>
	</div>
</form>
<!-- Modal -->
	    ...
	    <div class="modal-content">
	      <div class="modal-header">
			...
			<div class="modal-body" style="height:400px; overflow-y: scroll !important;">
				...
				<div class="form-group">
					<label class="col-md-4 control-label baja" for="passwordinput"><span class="aterisco">*</span>Precio neto: </label>
					<div class="col-md-6">
						<?php echo $this->Form->input('precio', array("class"=>"form-control requerido", "label"=>false, 'placeholder'=>'Precio neto'));?>
					</div>
				</div>
...
<script>
	...
	$(document).ready(function(){
		...
		$("#monto").change(function(){
			...
			$(this).val(accounting.formatMoney(eval(descuentoPeso), "", 0, ".", "."))
			tipoImpuesto = $("input[type='radio']:checked").val();
			if(tipoImpuesto != 1 && tipoImpuesto != 2)
			{
				nonbretipoImpuesto = "#tipo_documento"+tipoImpuesto;
		...
		$(document).on('click','.editar',function(){
			...
			if($('input:radio[name="data[tipo_documento]"]:checked').val() == 19){
				$("#afecto1" ).prop( "checked", true );
				$("#afecto2").attr("disabled", true);

			}	else{
				$("#afecto2").attr("disabled", false);
			}
		});
