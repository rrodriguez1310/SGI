...
<form class="form-horizontal" method="post" id="fileinfo" name="fileinfo" role="form">
	...
	<div class="form-group">
		<label class="col-md-4 control-label baja" for="passwordinput">Descuento $ : </label>
		<div class="col-md-6">
			<?php echo $this->Form->input('monto', array("class"=>"col-xs-4 form-control", "label"=>false, 'placeholder'=>'descuento %', 'default'=>number_format($comprasProductosTotale['ComprasProductosTotale']['descuento_total'], 0, '', '.')));?>
		</div>
	</div>
	...
	<div class="form-group">
		<div class="col-md-6 col-md-offset-4">
			<div class="alert alert-info">
				...
				<h4> Impuesto:<span class="iva"> $ <?php echo number_format($comprasProductosTotale["ComprasProductosTotale"]["total"] - $comprasProductosTotale["ComprasProductosTotale"]["neto_descuento"], 0, '', '.'); ?> </span></h4>
	...
	<div class="form-group">
		<div class="col-md-6 col-md-offset-4">
			<button type="submit" class="btn-block btn btn-primary btn-lg generarOrden"><i class="fa fa-file-text-o"></i> Editar Documento</button>
		</div>
	</div>
</form>
<!-- Modal -->
	      ...
	      <div class="modal-header">
			...
			<div class="modal-body" style="height:400px; overflow-y: scroll !important;">
				...
				<div class="form-group">
					...
					<div class="col-md-6">
						<?php echo $this->Form->input('codigoPresupuestario', array("type"=>"select", "options" =>array(""=>""), "class"=>"requerido codigoPresupuestario requerido", "label"=>false, 'placeholder'=>'Codigo Presupuestario'));?>
					</div>
				</div>
			</div>
...
<div class="modal fade" id="prompAvisoTrabajador" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <div class="modal-content"> 
      ...
      <div class="modal-body">
      		<div class="alert alert-danger" role="alert">
				<strong> La empresa que selecciono tambien es un trabajador activo</strong>
			</div>
      </div>
      <!--div class="modal-footer">
...
<input type="hidden" name="CorreoHHRR" id="CorreoHHRR" value="">
...
<script>
	...
	$(document).ready(function() {
		...
		$('#company_id').change(function(){
			...
			$.ajax({
		    type: "GET",
		    url:'<?php echo $this->Html->url(array("controller"=>"servicios", "action"=>"validaEmpresaTrabajador"))?>',
		    ...
		    dataType: "json",
				success: function( data ) {
					...
					{
						$('#CorreoHHRR').val(data)
						$('#prompAvisoTrabajador').modal('show');
					}else{
						$('#CorreoHHRR').val('')
					}
				}
	...
	$("#fileinfo").submit(function(e){
		...
		$.ajax({
			url: "<?php echo $this->Html->url(array('controller'=>'Servicios', 'action'=>'addDocumentoTributario')); ?>",
			type: "POST",
			data: fd,
			...
			if(data != "")
			{
				var CorreoHHRR = $('#CorreoHHRR').val();
				if(CorreoHHRR == 1){
					$.ajax({
						type: "GET",
						url:'<?php echo $this->Html->url(array("controller"=>"servicios", "action"=>"validaEmpresaTrabajadorEnviaCorreo"))?>',
						data: {
							nDocumento:$('#numero_documento').val(), 
			...
			if($("#cantidad").val() != "" && $("#descripcion").val() != "" && $("#codigoPresupuestario").val() != "" && $("#precio").val() != "" && $("#dimUno").val() != "")
			{
				...
				$.post(destino, datos, function (data) {
					...
					if(obj.cantidad != "")
					{
						...
						tdHtml = "<td class='eliminar'> <input type='hidden' value='"+obj.idProducto+"' name='idProducto[]'> <input type='hidden' name='cantidad[]' value='"+obj.cantidad.replace(/\./g,'')+"'>"+obj.cantidad+"</td>";
	...
	$("#tipo_documento17, #tipo_documento18, #tipo_documento19, #tipo_documento20 , #tipo_documento21, #tipo_documento22").click(function(){
		...
		{ 
			...
			if(valorEnPesosConDescuento != 0)
			{
				...
				//iva = eval(valorEnPesos) * 0.19; //eval(valorEnPesosConDescuento) * 0.19
