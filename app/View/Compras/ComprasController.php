...
define("SAP_ATTACH_DIR", "\\\\CDF-SQL01\\carpetas compartidas\\attachments\\TEST");
define("SGI_ATTACH_DIR", "/mnt/TEST_SAP23");
define("USAP", "manager");
define("PSAP", "manager");
...
*/
	...
	public function enviar_compra_sap(){
		...
		if(!empty($this->request->data["Sap"])){
			...
			/*$this->request->data["Sap"]["DocType"] = $this->request->data["sap_doc_type_id"];
			$this->request->data["Sap"]["Indicator"] = $this->request->data["sap_indicator_id"];
			$this->request->data["Sap"]["CardCode"]	= $this->request->data["sap_card_code_id"];*/
			...
			if($datosSap) {
				...
				if(isset($this->request->data["Sap"]["Attachments2_Lines"])) {
					...
					if (isset($archivoSap["AbsoluteEntry"])) {
						...
						$respuestaArchivo = " OBS: ".$archivoSap["message"]["value"];
					}
				}
	...
	*/
	...
	public function buscar_compras_ingresado_sap()
	{
		...
		$compras = $this->ComprasProductosTotale->find('all', array(
			//'conditions'=>array('ComprasProductosTotale.created LIKE' => "%2014%", 'ComprasProductosTotale.compras_estado_id'=>array(4,8)),
			//buscar_compras_ingresado_sap
			'conditions'=>array(
				'ComprasProductosTotale.created BETWEEN ? AND ?' => array('2014/01/01',$nuevafecha),
	...
	public function lista_email_json()
	{
		...
		$this->response->type('json');
		$this->loadModel("Email");
		$listaEmail = $this->Email->find("all", array("conditions"=>array("Email.informe != "=>"abonados")));
