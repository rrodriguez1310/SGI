<div class="center col-md-8 col-md-offset-2" ng-controller="controllerEstado" ng-init="respuesta(<?php echo $comprasTarjeta['ComprasTarjeta']['id']?>)">
	...
	<?php 
	if( $comprasTarjeta['ComprasTarjeta']['compras_tarjetas_estado_id']==1 || 
...
<div >
		<div class="row" >
			...
			<div class="col-sm-4 col-md-offset-2"></div>
				<div class="col-sm-4">
					<button type="button" class="btn btn-info envio" ng-click="estado(<?php echo $comprasTarjeta['ComprasTarjeta']['id']?>)"><i class="fa fa-spinner fa-spin esconde" ></i> Guardar</button>
				</div>
			</div>
		</div>
	</div>
...
<div class="center col-md-8 col-md-offset-2">
	<div class="row">
		...
		if( $comprasTarjeta['ComprasTarjeta']['compras_tarjetas_estado_id']==3){?>
			<div class="col-sm-4">	
...
?>
<style>
    #tarjeta_estado_id:invalid { color: black; }
    #tarjeta_estado_id.colorMeBlue.noValue,
    #tarjeta_estado_id.colorMeBlue option:first-child {
        color: #cccccc;
    }
...
$('#tarjeta_estado_id').on('change', function(){
        var $this = $(this);
...
$(".envio").on('click', function(event){
    if($('#tarjeta_estado_id').val().length != 0){
        $('.esconde').show();
    }else{
        $('.esconde').hide();
    }
