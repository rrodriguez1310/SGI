...
<form class="form-horizontal" method="post" id="fileinfo" name="fileinfo" role="form">
	<fieldset>
		...
		<div class="form-group">
			...
			<div class="col-md-6">
				<?php echo $this->Form->input('titulo', array("class"=>"col-xs-4 form-control", "label"=>false, 'placeholder'=>'titulo', 'default'=>$comprasProductosTotale["ComprasProductosTotale"]["titulo"]));?>
			</div>
		</div>
	...
	<?php echo $this->Form->hidden('clonar', array('default'=>2));?>
	<?php echo $this->Form->hidden('eliminar');?>

	<div class="form-group">
		<div class="col-md-6 col-md-offset-4">
			<div class="alert alert-info">
				<h4> Neto s/d:<span class="neto_sd"> $ <?php echo number_format(array_sum($netoSinDescuento) - array_sum($descuentosProductos), 0, '', '.'); ?></span></h4>
				<?php echo $this->Form->hidden('neto_descuento', array('default'=>$comprasProductosTotale['ComprasProductosTotale']['neto_descuento']));?>
				...
				<h4> Neto c/d:<span class="netoCd"> $ <?php echo number_format($comprasProductosTotale["ComprasProductosTotale"]["neto_descuento"], 0, '', '.'); ?> </span></h4>
				<h4> Iva:<span class="iva"> $ <?php echo number_format($comprasProductosTotale["ComprasProductosTotale"]["total"] - $comprasProductosTotale["ComprasProductosTotale"]["neto_descuento"], 0, '', '.'); ?> </span></h4>
	...
	<div class="form-group">
		<div class="col-md-6 col-md-offset-4">
			<button type="submit" class="btn-block btn btn-primary btn-lg generarOrden"><i class="fa fa-file-text-o"></i> Editar requerimiento</button>
		</div>
	</div>
</form>
<!-- Modal -->
<div class="modal fade" id="ingresoProducto" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog" style="width:800px;">
  	...
  	<!--form id="formProducto" role="form"  method="Post" action="/servicios/compraUnitario"-->
	      ...
	      <div class="modal-header">
			...
			<div class="modal-body" style="height:400px; overflow-y: scroll !important;">
				...
				<div class="form-group">
					...
					<div class="col-md-6">
						<?php echo $this->Form->hidden('idProducto');?>
...
<script>
	...
	$("#fileinfo").submit(function(e){
		...
		$.ajax({
			...
			if(data != "")
			{
				var url = "<?php echo $this->Html->url(array('controller'=>'compras', 'action'=>'index'));?>"
				if($("#eliminar").val() == 1)
				{
					url = window.location
				}
	...
	$(document).ready(function(){
    	...
    	$(document).on('click','.clsEliminarFila',function(event){
    		...
    		if(idProducto != "")
    		{
				...
				if(adjunto == "")
				{
					alert("Antes de eliminar un producto ingrese el Archivo adjunto");
					return false;
				}
				
				if(confirm('¿El producto sera eliminado?')){
				    ...
				    //el usuario no quiere eliminar sus datos
				    return false;
				}
			...
			if($("#cantidad").val() != "" && $("#descripcion").val() != "" && $("#codigoPresupuestario").val() != "" && $("#precio").val() != "" && $("#dimUno").val() != "")
			{
				...
				$.post(destino, datos, function (data) {
					...
					if(obj.cantidad != "")
					{
						...
						var tdHtml = ""
						tdHtml = "<td class='eliminar'> <input type='input' value='"+obj.idProducto+"' name='idProducto[]'> <input type='hidden' name='cantidad[]' value='"+obj.cantidad+"'>"+obj.cantidad+"</td>";
		...
		$("#monto").change(function(){
			...
			$("#monto").val(accounting.formatMoney(descuentoPeso, "", 0, ".", "."));
			return false;
		});
...
<script>
	...
	$(document).ready(function(){
		...
		$(document).on('click','.editar',function(){
			...
			$("#descripcion").val(descripcion);
			$("#precio").val(precioUnitario);
			$("#empaque").select2({allowClear: true,width:'100%'}).select2('val',empaque);
			$("#dimUno").select2({allowClear: true,width:'100%'}).select2('val',dimUno);
						...
						{	
							$.each(data, function(i, item){
								...
								if(dimUno != "" && codigoPresupuestario != "")
								{
									if(codigoPresupuestario == item.Id)
									{
										$("#codigoPresupuestario").select2({allowClear: true,width:'100%'}).select2('val',codigoPresupuestario);
									}
								}
							});
			...
			$("#dimDos").select2({allowClear: true,width:'100%'}).select2('val',dimDos);
			$("#dimTres").select2({allowClear: true,width:'100%'}).select2('val',dimTres);
			$("#dimCuatro").select2({allowClear: true,width:'100%'}).select2('val',dimCuatro);
			$("#dimCinco").select2({allowClear: true,width:'100%'}).select2('val',dimCinco);
			$("#proyecto").select2({allowClear: true,width:'100%'}).select2('val',proyecto);
