app.controller('cargosIndex', function ($scope, $q, $filter, cargosService, Flash){
    ...
    var nombreHost = window.location.pathname.split("/")[1];
    if (nombreHost == 'sgi-v3' || nombreHost == 'qa') {
    ...
    $scope.gridOptions.columnDefs = [
        ...
        {name:'descripcionCargo', cellTemplate:'<div class="text-center">  <a href="'+nombreHost+'/files/descripcion_cargo/{{row.entity.descripcionCargo}}" target="_blank">{{(row.entity.descripcionCargo) != null ? "ver adjunto" : ""}}</a></div>' },
    ];
...
app.controller('cargosAdd', function ($scope, $q, $filter, cargosService, areasService, gerenciasService, cargosNivelResponsabilidadesService, cargosFamiliasService, areasFactory, $window, Flash){
    ...
    $q.all(promesas).then(function (datos){
        ...
        $scope.registrarCargo = function(){
        	...
        	cargosService.registrarCargoAdd({ Cargo : $scope.formulario.Cargo,  Archivo : $scope.formulario.File}).success(function (data){
        		//console.log(data);
        		if(data.estado == 1){
    ...
    $scope.$watch("idCargo", function (idCargo){
	    ...
	    $q.all(promesas).then(function (datos){
	        ...
	        $scope.eliminarAdjunto = function(){
		    	$scope.formulario.Cargo.descripcion_cargo = null;
		    	console.log($scope.formulario.Cargo);
		    	$scope.$apply();
		    };
	        	...
	        	cargoEdit = {
	        		Cargo : {
	        			...
	        			area_id : $scope.formulario.Cargo.area_id,
	   					...
	   					cargos_familia_id : $scope.formulario.Cargo.cargos_familia_id,
	   					descripcion_cargo : $scope.formulario.Cargo.descripcion_cargo
	        		},
	        		Archivo : {
	        			descripcion : ($scope.formulario.File != undefined ? $scope.formulario.File.descripcion : false)
	        		}
	        	};


	        	cargosService.registrarCargoEdit(cargoEdit).success(function (data){
	        		if(data.estado == 1){
	        			$window.location = host+"cargos";
	        		}else{
	        			Flash.create('danger', data.mensaje, 'customAlert');
