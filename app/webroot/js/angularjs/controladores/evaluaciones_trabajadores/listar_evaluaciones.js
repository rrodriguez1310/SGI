...
app.controller('evaluacionesIndex', ['$scope', '$rootScope', '$http', '$filter', '$location', 'uiGridConstants', 'i18nService', function ($scope, $rootScope, $http, $filter, $location, uiGridConstants, i18nService) {
    ...
    $http.get(host+'evaluaciones_trabajadores/listado_evaluaciones/').success(function(data) {      
        ...
        $scope.estadoEvaluacion = data.estadoEvaluacion;
       

        if(!$scope.inicio){

            $scope.gridApi.selection.on.rowSelectionChanged($scope,function(row){
                $scope.idTrabajador = null;            

                if (angular.isNumber(row.entity.evaluacion_id)) {
                    if( (row.entity.estado_id == 5 || row.entity.estado_id == 6 || row.entity.estado_id == 7) && $scope.estadoEvaluacion == 1){
                        ...
                        $scope.btnlistado_historico = false;
                        ...
                        $scope.btnlistado_historico = false;
                        $scope.idTrabajador = row.entity.trabajador_id;

                    }else if(row.entity.estado_id == 4 || row.entity.estado_id == 9){
                    ...
                    }else if ($scope.estadoEvaluacion == 1) {
                ...
                console.log($scope.idTrabajador);

                if ($scope.idTrabajador == null) {
                    $scope.listaTrabajador = function() {
                        $scope.idTrabajador = $scope.id;
                        window.location.href = host + "evaluaciones_trabajadores/view/" + $scope.idTrabajador
                        
                    };
                } else {
                    $scope.listaTrabajador = function() {
                        window.location.href = host + "evaluaciones_trabajadores/historico/" + $scope.idTrabajador
...
app.controller('miDesempenoIndex', ['$scope', '$rootScope', '$http', '$filter', '$location', 'uiGridConstants', 'i18nService', 'evaluacionesService', 'Flash', function ($scope, $rootScope, $http, $filter, $location, uiGridConstants, i18nService, evaluacionesService, Flash ) {
    ...
    $http.get(host+'evaluaciones_trabajadores/listado_desempeno/').success(function(data) {
        ...
        $scope.anioEvaluado = data.anio_evaluado;
        $scope.anioAEvaluar = data.anio_ocd;
...
app.controller('evaluacionesConsolidadoIndex', ['$scope', '$rootScope', '$http', '$filter', '$location', 'uiGridConstants', 'i18nService', 'evaluacionesService', 'constantes', function ($scope, $rootScope, $http, $filter, $location, uiGridConstants, i18nService, evaluacionesService, constantes) {
    ...
    $scope.gridOptions.columnDefs = [
        ...
        {name:'ultimo_cambio', displayName:'Último cambio',visible:false}
    ];
...
//all 
app.controller('evaluacionesIndexAllBy', ['$scope', '$rootScope', '$http', '$filter', '$location', 'uiGridConstants', 'i18nService', function($scope, $rootScope, $http, $filter, $location, uiGridConstants, i18nService) {
    $scope.inicio = false;
    $scope.loader = true;
    $scope.tablaDetalle = false;
    $scope.cargador = loader;
    ...
    $scope.getCellFilterDateAnio = function() {
        return 'date:\'yyyy\'';
    };
    $scope.idDate = 1;
    $scope.gridOptions.columnDefs = [
        { name: 'anio', displayName: 'Año Evaluado' },
        { name: 'cargo', displayName: 'Cargo' },
        { name: 'estado_id', displayName: 'Id Estado', visible: false },
        { name: 'estado', displayName: 'Estado' }
    ];
    ...
    $scope.datosEvaluacion = function(idEvaluacion) {
        ...
        $http.get(host + 'evaluaciones_trabajadores/listado_historico/' + idEvaluacion).success(function(data) {
            ...
            if (data.listaTrabajadores.length == 0) {
                $scope.nombreTrabajador = null;
            } else {
                $scope.nombreTrabajador = data.listaTrabajadores[0].nombre;
            }
...
app.controller('evaluacionesEdit', function ($scope, $http, evaluacionesService, Flash, $window, $location, $rootScope, $filter, $anchorScroll) {
    ...
    $scope.datosEvaluacion = function(idEvaluacion){
        ...
        evaluacionesService.getEvaluacion(idEvaluacion).success(function (evaluacionData){
            ...
            if(evaluacionData.EvaluacionesTrabajadore.evaluaciones_estado_id == 1 || evaluacionData.EvaluacionesTrabajadore.evaluaciones_estado_id == 0){
                $scope.pasoCompetencias =  true;
                $scope.pasoObjetivos =  false;
                $scope.pasoDialogo =  false;
            ...
            $scope.simboloUnidad            = evaluacionData.unidadMedida;
            ...
            console.log($scope.simboloUnidad);
...
/*$scope.cambioCompetencias = [];
    ...
    $scope.siguienteEvaluacion = function(evaluacion){
        ...
        }else if($scope.formulario.EvaluacionesTrabajadore.evaluaciones_estado_id==5 || $scope.formulario.EvaluacionesTrabajadore.evaluaciones_estado_id==0){
...
app.controller('evaluacionesView', function ($scope, $http, evaluacionesService, Flash, $window, $location, $rootScope, $filter, $anchorScroll) {
    ...
    $scope.datosEvaluacion = function(idEvaluacion){
        ...
        evaluacionesService.getEvaluacionAll(idEvaluacion).success(function(evaluacionData) {
            $scope.loader = false;
            $scope.evaluacion = true;
            $scope.evFinalizada = false;
...
app.controller('evaluacionesGraficos', function ($scope, $q, $filter, Flash, evaluacionesService, uiGridConstants, servicios){
    ...
    $scope.mostrarSeccion = function(eleccion){
        ...
        switch(eleccion){
            ...
            break;
            case 2:
                $scope.seccionDistribucion = true;
            break;
            case 3:
                $scope.seccionDistribucionDesempeno = true;
            break;
            case 4:
                $scope.seccionDistribucionCompetencias = true;
            break;
            case 5:
                $scope.seccionComparativo = true;
            break;
            case 6:
                $scope.seccionComparativoGerencias = true;
            break;
            case 7:
                $scope.seccionDistribucionNivel = true;
            break;
            case 8:
                $scope.seccionDistNivelCompetencias = true;
            break;
            case 9:
                $scope.seccionDistNivelFamlia = true;
            break;
        }
    };
