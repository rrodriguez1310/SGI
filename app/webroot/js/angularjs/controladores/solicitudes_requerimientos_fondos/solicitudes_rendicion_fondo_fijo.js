app.controller('controllerRequerimientos', ['$scope', '$http', '$filter', 'solicitudesRequerimiento', function($scope, $http, $filter, solicitudesRequerimiento) {
   ...
   // $scope.guardarRendicion = [];
    ...
    solicitudesRequerimiento.listaTodosFondoFijo().success(function(data){
        //$scope.loader = false
        if(data.length > 0){
            $scope.loader = false
       ...
       $scope.gridApi.selection.on.rowSelectionChanged($scope,function(row){
            if(row.isSelected == true){
                if(row.entity.id){
                    ...
                    if (row.entity.estado == "Rechazado Area" || 
                        ...
                        $scope.btnsolicitud_rendicion_totalesdelete=true;
                        $scope.btnsolicitud_rendicion_totalesedit=true;
                    ...
                    } else if( row.entity.estado == "Aprobado Area" || 
                               row.entity.estado == "Aprobado Finanza" ){
                       /* $scope.btnsolicitud_rendicion_totalesdelete=true;
                        /*$scope.btnsolicitud_rendicion_totalesedit=true;*/
                       ...
                       // $scope.btnsolicitud_rendicion_totalesdelete=false;
                       // $scope.btnsolicitud_rendicion_totalesedit=false;
        ...
        $scope.confirmacion = function(){
            if(angular.isDefined($scope.id)){
                window.location.href = host+"solicitud_rendicion_totales/delete/"+$scope.id
            }
        };
