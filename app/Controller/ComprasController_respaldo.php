 ...
 */
	...
	public function aprobar_compra($id)
	{
		...
		if (!empty($id)) 
		{
			...
			if ($this->ComprasProductosTotale->save($this->request->data)) 
			{
				...
				$Email->template('orden_compra_aprobada');
		...
		if(!empty($this->data))
		{
			...
			if($this->data["id_rechazo"]) 
			{
				...
				if ($this->ComprasProductosTotale->save($this->request->data)) 
				{
					...
					//pr($idUsuarioRequerimientoCompra); exit;
					...
					$Email->emailFormat('html');
					$Email->template('orden_compra_rechazada');
	...
	public function aprobar_compra_sap()
	{
		...
		if (!empty($id)) 
		{
			...
			if ($this->ComprasProductosTotale->save($this->request->data)) 
			{
				...
				//$email[] = $idUsuarioRequerimientoCompra["User"]["email"];
				$emailCorto = explode(".", $idUsuarioRequerimientoCompra["User"]["email"]);
				$email[] = $emailCorto[0].'.cl';
	...
	public function todos_sap()
	{
		...
		if ($this->request->is(array('post', 'put'))) 
		{
			...
			{
				$conditions = array('ComprasProductosTotale.created BETWEEN ? and ?'=> array($this->request->data["TodosSap"]["fecha_desde"], $this->request->data["TodosSap"]["fecha_hasta"]));
			}
			else
			{
