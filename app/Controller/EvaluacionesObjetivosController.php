		...
		$listadoTrabajadores = $this->Trabajadore->find("all", array( 
			"conditions"=> array(
				"OR" => array(
					...
					array( "Trabajadore.tipo_contrato_id" => array(1,4), "( (EXTRACT(YEAR FROM DATE('$anio'))) - 1 ) > 2014")		// En adelante, indefinido y part-time
					),
 ...
 */
	...
	public function listado_objetivos() {
		...
		$anioActual = $this->EvaluacionesAnio->find("first", array( 
        ...
        $anioAEvaluar = isset($anioActual["EvaluacionesAnio"]["anio_evaluado"]) ? $anioActual["EvaluacionesAnio"]["anio_evaluado"] : null;
      ...
      //  pr($datosUser["User"]["trabajadore_id"]);
       // exit;
        ...
        $evaluaciones = $this->EvaluacionesTrabajadore->find("first", array(
            ...
            "recursive"=>1,
            "order" => "EvaluacionesTrabajadore.anio DESC",
            "limit"=>1
            )
        );
        ...
        if($datosUser["User"]["trabajadore_id"] == 110 || $datosUser["User"]["trabajadore_id"] == 501)
            $nodoInicial = true;
        ...
        $ocdDefinido = true;
        //pr($datosJefe["Jefe"]["id"]);
        if( ($ocdDefinido || $nodoInicial) && $anioAEvaluar ){
    		$anioActual = date("Y")-1;
    		$trabajadoresPorJefe = $this->Trabajadore->find("all", array(
    			"fields"=>array( "Trabajadore.id", "Trabajadore.nombre", "Trabajadore.apellido_paterno", "Trabajadore.fecha_indefinido", "Cargo.nombre",//"CargosFamilia.nombre", // se guarda? o se evalua con el actual
                    ...
                    "EvaluacionesObjetivo.created", "EvaluacionesObjetivoEstado.nombre", "EvaluacionesTrabajadore.anio","EvaluacionesAnio.estado"
    				),
    			"joins" => array( array("table" => "evaluaciones_trabajadores", "alias" => "EvaluacionesTrabajadore", "type" => "LEFT", "conditions" => array("EvaluacionesTrabajadore.trabajadore_id = Trabajadore.id AND EvaluacionesTrabajadore.anio = $anioAEvaluar")),
                    ...
                    array("table" => "evaluaciones_anios", "alias" => "EvaluacionesAnio", "type" => "LEFT", "conditions" => array("EvaluacionesTrabajadore.evaluaciones_anio_id = EvaluacionesAnio.id")),
    				),
    			"conditions"=>array(
    				"Trabajadore.jefe_id" => $datosJefe["Jefe"]["id"], //**************** cambiar si existe evaluador (evaluacion creada)
    				"Trabajadore.estado" => "Activo",
    				"Trabajadore.tipo_contrato_id" => array(1,4),
                    "OR" => array(
                        array("EvaluacionesAnio.estado <" => 3),      // Definicion, Calibrado
                        array("EvaluacionesAnio.estado" => null)                 // No iniciado,
                        ),
    				//"Trabajadore.fecha_indefinido <=" => $anioActual.'-03-31', // indefinido, part-time al 31-03-año anterior    				
                    "OR" => array(
    		...
    		foreach ($trabajadoresPorJefe as $trabajador) {
    			...
    			$listadoTrabajadores[] = array_merge( $trabajador["Trabajadore"], 
    				array( "evaluacion_id" => $trabajador["EvaluacionesTrabajadore"]["id"],
                        ...
                        "estado" => (isset($trabajador["EvaluacionesObjetivoEstado"]["nombre"]))? $trabajador["EvaluacionesObjetivoEstado"]["nombre"] : 'No Iniciado' ,
    ...
    public function listado_calibrador() {
       ...
       if($datosUser["User"]["trabajadore_id"] == 403)
            $datosUser["User"]["trabajadore_id"] = 313;
 ...
 */
	...
	public function add_json($id=null) {
        ...
        if($trabajador["JefeTrabajadore"]["id"] == 110 || $trabajador["JefeTrabajadore"]["id"] == 501)
            $nodoInicial = true;        
 ...
 */
	...
	public function edit_json($id = null, $etapa = 1) {
		...
		$datosTrabajadorG = $this->Trabajadore->find("all", array(
			"fields"=>array("Trabajadore.id","Trabajadore.nombre", "Trabajadore.apellido_paterno", "Trabajadore.apellido_materno", "Trabajadore.email", "Trabajadore.foto", "Cargo.nombre", "CargosFamilia.nombre", "CargosFamilia.id", "JefeTrabajadore.id", "JefeTrabajadore.nombre", "JefeTrabajadore.apellido_paterno", "JefeTrabajadore.email", "CalibradorJefe.id", "CalibradorJefe.nombre", "CalibradorJefe.apellido_paterno", "CalibradorJefe.email", "EvaluacionesTrabajadore.anio"),
        ...
        if($datosTrabajadorG[0]["JefeTrabajadore"]["id"] == 110 || $datosTrabajadorG[0]["JefeTrabajadore"]["id"] == 501)
            $nodoInicial = true;
		...
		foreach ($datosTrabajadorG as $trabajador) {			
			$datosTrabajador = array(
				...
				"foto" 				=> $this -> webroot . 'files' . DS . 'trabajadores' . DS . $trabajador["Trabajadore"]["foto"],
                "anio"              => $trabajador["EvaluacionesTrabajadore"]["anio"] ? $trabajador["EvaluacionesTrabajadore"]["anio"] : $anioEvaluado
				);
		}
		//ocd
    ...
    public function objetivos_consolidado_json($anio=null){
        ...
        $listadoAnios = array();
        $anioDefinidos  = $this->EvaluacionesAnio->find("all", array( 
            "conditions"=>array("estado >"=>0, "anio_evaluado >" => 2015),
            "order"=>"anio_evaluado DESC"
            )
        );
        foreach ($anioDefinidos as $anios){                        
            $listadoAnios[] = $anios["EvaluacionesAnio"];
            $anioActual[] = $anios["EvaluacionesAnio"]["anio_evaluado"];
        }
        $anioEvaluado = $anio_evaluado["EvaluacionesAnio"]["anio_evaluado"];
        ...
        $trabajadores = $this->EvaluacionesTrabajadore->find("all", array(
            ...
            "joins" => array( array("table" => "trabajadores", "alias" => "Trabajadore", "type" => "RIGHT", "conditions" => array("EvaluacionesTrabajadore.trabajadore_id = Trabajadore.id AND EvaluacionesTrabajadore.anio=$anio")),                
                ...
                array("table" => "evaluaciones_objetivos", "alias" => "EvaluacionesObjetivo", "type" => "LEFT", "conditions" => array("EvaluacionesObjetivo.evaluaciones_trabajadore_id = EvaluacionesTrabajadore.id AND EvaluacionesObjetivo.estado = 1 AND objetivos_comentario_id = 1")),
             ),
            "conditions" => array(
                "OR" => array(
                    array("Trabajadore.estado" => "Activo"),
                    array("EvaluacionesObjetivo.id !=" => null)
                    ),
                //"Trabajadore.estado" => "Activo",
                "Trabajadore.tipo_contrato_id" => array(1,4),
                "Trabajadore.fecha_indefinido <=" => $anio.'-12-31', // indefinido, part-time al 31-03-año anterior
                ),
            "recursive"=>1
            ));
        ...
        foreach ($trabajadores as $trab) {
            if(!empty($trab["EvaluacionesObjetivo"])){
                ...
                $listadoObjetivos[] = array_merge( array(
                        ...
                        "estadoObjetivos" => (!empty($trabOCD))? $trabOCD["estado0"] : 'No Iniciado'),
                        $trabOCD
                        );
            }else{
    ...
    public function validacion_automatica_ocd(){
        //tras un día se debe validar automaticamente.
        // todos los dias
        $this->layout = "ajax";
        $envioCorreo = '';
        ...
        $anioOCD = $this->EvaluacionesAnio->find("list", array( 
            "fields"=>array("EvaluacionesAnio.anio_evaluado"),
            "conditions" => array("EvaluacionesAnio.estado"=>1)
            )
        );

        $evaluacionesOCD = $this->EvaluacionesTrabajadore->find("list", array( 
            "conditions" => array("EvaluacionesTrabajadore.anio"=>$anioOCD)
            )
        );
        
        $diasValidacion = $this->EvaluacionesObjetivoEstado->find("list", array( 
            "fields"=>array("EvaluacionesObjetivoEstado.dias_plazo"),
            "conditions" => array("EvaluacionesObjetivoEstado.id"=>6)
            )
        );

        $diaAnterior = date('Ymd', strtotime('-'.$diasValidacion[6].' day', strtotime( date('Y-m-d') )));        
        
        $OCDs = $this->EvaluacionesObjetivo->find("list", array(             
            "conditions"=> array(                                 
                "EvaluacionesObjetivo.evaluaciones_trabajadore_id" => $evaluacionesOCD,
                "TO_CHAR(EvaluacionesObjetivo.created,'YYYYMMDD') < " => $diaAnterior, 
                "EvaluacionesObjetivo.evaluaciones_objetivo_estado_id" => 6
                ),
            "recursive"=>-1
            )
        );

        $this->EvaluacionesObjetivo->updateAll(
            array("EvaluacionesObjetivo.evaluaciones_objetivo_estado_id" => 7,
                //"EvaluacionesObjetivo.validado" => 1
                ),
           array( "EvaluacionesObjetivo.id" => $OCDs)
            /*array(                                 
                "EvaluacionesObjetivo.evaluaciones_trabajadore_id" => $evaluacionesOCD,
                "TO_CHAR(EvaluacionesObjetivo.created,'YYYYMMDD') < "=>date('Ymd'), 
                "EvaluacionesObjetivo.evaluaciones_objetivo_estado_id" => 6
                )*/
        );
        
        if (!empty($OCDs)){
            $datos = array("datos"=>implode(",", $OCDs));
            $Email = new CakeEmail("gmail");
            $Email->from(array('sgi@cdf.cl' => 'SGI'));
            $Email->to(array("ddiaz@cdf.cl" => "Diego Diaz" ));             
            $Email->subject("OCD validados automaticamente"); //$correo["asunto"]
            $Email->emailFormat('html');
            $Email->viewVars($datos);
            $Email->template("evaluacion_objetivos_autovalidacion");            
            $Email->send();        
        }
