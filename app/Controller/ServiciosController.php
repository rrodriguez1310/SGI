...
class ServiciosController extends AppController {
	...
	public function codigoPresupuestarioCompraTarjeta()
	{
		...
		*/
		...
		//{
			...
			if(!empty($this->params->query["valor"]))
			{
				...
				$codigoCorto = $this->Dimensione->findByCodigo($this->params->query["valor"]);
				...
				if(!empty($codigosPresupuestarios))
				{
					foreach($codigosPresupuestarios as $codigosPresupuestario)
					{
						...
						$codigoPresupuesto[] =  array( 
                            "Id"=>$codigosPresupuestario["CodigosPresupuesto"]["id"], 
                            "Nombre"=>$codigosPresupuestario["CodigosPresupuesto"]["codigo"] . ' - ' . $codigosPresupuestario["CodigosPresupuesto"]["nombre"]
                        ); 
					}
				}
			}
		...
		//{
			...
			//	pr($codigo[0]);
				...
				if(!empty($codigoCorto["Dimensione"]["codigo_corto"]))
				{
					...
					$idAgno = $this->Year->find("first", array(
						'conditions'=>array("Year.nombre"=>date('2018'))
					));
				...
				if(!empty($codigosPresupuestarios))
				{
					...
					{
						$codigoPresupuesto[] =  array( 
                            "Id"=>$codigosPresupuestario["CodigosPresupuesto"]["codigo"], 
                            "Nombre"=>$codigosPresupuestario["CodigosPresupuesto"]["codigo"] . ' - ' . $codigosPresupuestario["CodigosPresupuesto"]["nombre"]
                        ); 
					}
				}
...
/*	public function actualiza_tipo_cambios()
	...
	*/	
	...
	public function codigo_nombre(){
		$this->autoRender = false;
		$this->response->type("json");

		if($this->params->query['codigo']){
			
			$this->loadModel("CodigosPresupuesto");

			$resultado = str_replace("+", "", $this->params->query['codigo']);

			$nombre = $this->CodigosPresupuesto->find('first', array(
				'conditions'=>array('CodigosPresupuesto.codigo'=>trim($resultado)),
				'fields'=>array('CodigosPresupuesto.nombre')
			));

			if(!empty($nombre)){
				echo json_encode($nombre['CodigosPresupuesto']['nombre']);
			}else{
				echo json_encode('sin información');
			}
		}

	}

	public function gerencias_json(){
		header('Access-Control-Allow-Origin: *');
		header('Access-Control-Allow-Methods: GET, POST');
		header("Access-Control-Allow-Headers: X-Requested-With");
        $this->autoRender = false;      
