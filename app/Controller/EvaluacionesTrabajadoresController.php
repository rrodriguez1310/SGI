...
class EvaluacionesTrabajadoresController extends AppController {
	...
	public function trabajadoresAEvaluar($anio=null){
		...
		$listadoTrabajadores = $this->Trabajadore->find("all", array( 
			"conditions"=> array(
				"OR" => array(
					array( "Trabajadore.tipo_contrato_id"=>1, "Trabajadore.rut !=" => "17.260.361-0"), 	// 2015 tipo contrato solo indefinido. // filtro carla palma solicitado por rrhh para 2015					
 ...
 */
	...
	public function add_json($id) {
		...
		$anio_evaluado = $this->EvaluacionesAnio->find("first", array( 
			"fields"=>array("id","anio_evaluado"), 
	...
	public function add_evaluacion(){
		...
		if( array_key_exists( "id", $this->request->data["EvaluacionesTrabajadore"] ) ){
			...
			if( $this->EvaluacionesTrabajadore->exists() ) {					//edit
				...
				$anioEvaluado = $this->EvaluacionesAnio->find("first", array ("fields"=>"id", "conditions"=>array("EvaluacionesAnio.anio_evaluado"=>$this->request->data["EvaluacionesTrabajadore"]["anio"])));								
				$this->request->data["EvaluacionesTrabajadore"]["evaluaciones_anio_id"] = $anioEvaluado["EvaluacionesAnio"]["id"];				
		...
		if( isset($this->request->data["EnvioCorreo"]) ) {
			if($respuesta["estado"]==1){
				...
				if($this->request->data["EvaluacionesTrabajadore"]["evaluaciones_estado_id"]==4){  //enviado a calibracion 3 cambia a 4
					...
					$estado = $this->EvaluacionesEstado->find("first", array("fields"=>"dias_plazo","conditions"=> array("id"=>$this->request->data["EvaluacionesTrabajadore"]["evaluaciones_estado_id"]),"recursive"=>0));
					$anioEvaluado = $this->EvaluacionesAnio->find("first", array ( "fields"=>"anio_evaluado", "conditions"=>array("estado"=>2)));
 ...
 */
	...
	public function edit_json($id = null){
		...
		$datosTrabajadorG = $this->Trabajadore->find("all", array(
			"fields"=>array("Trabajadore.id","Trabajadore.nombre", "Trabajadore.apellido_paterno", "Trabajadore.apellido_materno", "Trabajadore.email", "Trabajadore.foto", "Cargo.nombre", "CargosFamilia.nombre", 
				"CargosFamilia.id", "JefeTrabajadore.id", "JefeTrabajadore.nombre", "JefeTrabajadore.apellido_paterno", "JefeTrabajadore.email", "Gerencia.nombre", "CalibradorJefe.id", "CalibradorJefe.nombre", 
		...
		//competencias 
		$dataCompetencias = $this->competencias($evaluacion["EvaluacionesCompetencia"], $datosTrabajador["cargos_familia_id"]);
		//competencias generales
		$dataComGeneral = $this->competenciasGenerales($evaluacion["EvaluacionesCompetenciasGenerale"]);
		...
		foreach ($evaluacion["EvaluacionesObjetivo"] as $key => $objetivo)
		{
			$porcObjetivos[] = array( "id" => $objetivo["objetivos_comentario_id"], 
				"nombre" => $objetivos["nombresClave"][$objetivo["objetivos_comentario_id"]], 
				"puntaje" => $objetivo["puntaje"],
				"porcentaje" => $objetivo["porcentaje_ponderacion"], /*round(($objetivo["puntaje"] / $maxObj) * 100 , 0 )*/
				"descripcion_objetivo" => $objetivo["descripcion_objetivo"]
				);
			if(!empty($evaluacion["EvaluacionesObjetivo"][$key]["simbolo_indicador"])){
				$evaluacion["EvaluacionesObjetivo"][$key]["simbolo_indicador"] = $unidadMedida[$objetivo["evaluaciones_unidad_objetivo_id"]];
			}
			
		}

		//dialogo
		$dialogos = $this->dialogo();			
		$anioEvaluado = $this->EvaluacionesAnio->find("first", array( 
			"fields"=>"anio_evaluado", 
			"conditions"=>array("estado"=>2)
			)
		);
		
		if(empty($anioEvaluado)){
			$anioEvaluado = $this->EvaluacionesAnio->find("first", array( 
				"fields"=>"anio_evaluado", 
				"conditions"=>array("estado"=>2),
		...
		$diasPlazo = 0;
		$estado = $this->EvaluacionesEstado->find("first", array(
			"fields"=>"dias_plazo",
			"conditions"=> array("id"=>$evaluacion["EvaluacionesTrabajadore"]["evaluaciones_estado_id"]),
			"recursive"=>0
			)
		);
		...
		$editEvaluacion = array_merge(
			...
			array(				
				...
				"objetivosClave"		=>  $objetivos["objClave"],
				"dialogosDesempeno"		=>  $dialogos,
	...
	public function unidadMedida(){
		...
		$dataUnidadMedida = $this->EvaluacionesUnidadObjetivo->find("list", array( 			
			"fields"		=> array("EvaluacionesUnidadObjetivo.id", "EvaluacionesUnidadObjetivo.simbolo"),
	...
	public function competencias($evaluacionCompetencias, $cargosFamilia){
		$this->loadModel("Competencia");
		$this->loadModel("TitulosCompetencia");
		$dataCompetencias = "";
		if(!empty($evaluacionCompetencias)){ // nueva
			//foreach ($evaluacion["EvaluacionesCompetencia"] as $key =>  $competencia) {
			foreach ($evaluacionCompetencias as $key =>  $competencia) {
			...
			$evaluacionesComEspecificas = $this->TitulosCompetencia->find("all", array(
				...
				"conditions"=>array(
					array( "GruposCompetencia.cargos_familia_id" => $cargosFamilia,
						"TitulosCompetencia.estado" => 1,
						"GruposCompetencia.estado" => 1, 
	...
	public function competenciasGenerales($evaluacionCompetenciasGenerales){
		$this->loadModel("CompetenciasGenerale");
		
		$dataComGeneral = "";
		if(!empty($evaluacionCompetenciasGenerales)){ // nueva
			//foreach ($evaluacion["EvaluacionesCompetenciasGenerale"] as $key =>  $competenciaGeneral) {
			foreach ($evaluacionCompetenciasGenerales as $competenciaGeneral) {
			...
			$evaluacionesComGenerales = $this->CompetenciasGenerale->find("all", array(
				"fields"=>array("TitulosCompetencia.nombre", "CompetenciasGenerale.nombre", "CompetenciasGenerale.id","CompetenciasGenerale.secuencia"),
				"conditions"=>array( array( "TitulosCompetencia.estado" => 1, 
					"CompetenciasGenerale.estado" => 1),
					),
				"recursive"=>0,
				"order" => array("CompetenciasGenerale.secuencia ASC"),
				));
	...
	public function objetivos(){
		...
		return array("objClave"=>$objClave,"nombresClave"=>$nombresClave);
	}
 ...
 */
	public function formulario($id){
		...
		if($formulario["EvaluacionesTrabajadore"]["evaluador_trabajadore_id"] == 110 ||$formulario["EvaluacionesTrabajadore"]["evaluador_trabajadore_id"] == 501)
	...
	public function listado_evaluaciones(){
		...
		$anioOCD = $this->EvaluacionesAnio->find("first", array( 
			"fields"=>"anio_evaluado", 
			"conditions"=>array("estado"=>1)
			)
		);

		$anioActual = isset($anioEvaluado["EvaluacionesAnio"]["anio_evaluado"])? $anioEvaluado["EvaluacionesAnio"]["anio_evaluado"] : date("Y")-1;
		if (isset($anioEvaluado["EvaluacionesAnio"]["anio_evaluado"])) {
			$anioActual = $anioEvaluado["EvaluacionesAnio"]["anio_evaluado"];
			$estadoEvaluacionDesem = 1;
		}
		else {
			$anioActual = date("Y")-1;
			$estadoEvaluacionDesem = 0;
		}
		...
		//evaluacion finalizada evaluador
		if($datosTrabajador["User"]["trabajadore_id"] == 403)
			$datosTrabajador["User"]["trabajadore_id"] = 272;
		...
		$trabajadoresPorJefe = $this->EvaluacionesTrabajadore->find("all", array(
			"fields"=>array( "Trabajadore.id", "EvaluacionesAnio.estado", "Trabajadore.nombre", "Trabajadore.apellido_paterno", "Trabajadore.fecha_ingreso", "Trabajadore.estado", "Cargo.nombre", "CargosFamilia.nombre", 
				"EvaluacionesTrabajadore.id", "EvaluacionesTrabajadore.fecha_inicio", "EvaluacionesTrabajadore.fecha_modificacion", "EvaluacionesTrabajadore.fecha_reunion", "EvaluacionesTrabajadore.lugar_reunion", 
				"EvaluacionesTrabajadore.hora_reunion", "EvaluacionesTrabajadore.evaluaciones_estado_id", "EvaluacionesEstado.id", "EvaluacionesEstado.nombre"),
			"joins"=> array( array("table" => "cargos_familias", "alias" => "CargosFamilia", "type" => "LEFT", "conditions" => array("Cargo.cargos_familia_id = CargosFamilia.id"))),				
			"conditions"=> array(
				"EvaluacionesTrabajadore.anio" => $anioActual,
				"EvaluacionesTrabajadore.evaluador_trabajadore_id" => $datosTrabajador["User"]["trabajadore_id"],
				"Trabajadore.estado" => 'Activo'),
		...
		if(!empty($trabajadoresPorJefe)){
			foreach ($trabajadoresPorJefe as $trabajador) {
				$listaTrabajadores[] = array(
					...
					"cargo" 				=> $serv->capitalize($trabajador["Cargo"]["nombre"]), //$nombreCargo,
					"familia_cargo"		=> $serv->capitalize($trabajador["CargosFamilia"]["nombre"]), //$nombreFamiliaCargo,
		...
		$dataJefeEvaluador = array(
			...
			"anio_evaluado"		=> $anioActual,
			"usuario_evaluado" 	=> (!empty($evaluaciones))? 1: 0,
			"estadoEvaluacion" => $estadoEvaluacionDesem
		);

		$this->set("evaluacionesPorJefe", $dataJefeEvaluador);
	}

	public function listado_historico($id){
		...
		//pr($anioEvaluado);exit;
		$anioActual = isset($anioEvaluado["EvaluacionesAnio"]["anio_evaluado"])? $anioEvaluado["EvaluacionesAnio"]["anio_evaluado"] : null;		
		//$anioAnterior = $anioActual-2;
		//Usuario-Trabajador
		$datosTrabajador = $this->User->find("first", array(
			"fields"=>array("User.trabajadore_id"),
			"conditions"=>array("User.id" => $this->Session->read('PerfilUsuario.idUsuario')),
		...
		//if(!empty($evaluaciones) ||  $datosJefe["Trabajadore"]["id"] == 110 || $datosJefe["Trabajadore"]["id"] == 111){	// usuario evaluado?
			// listado trabajadores
			//pr($datosTrabajador["User"]["trabajadore_id"]);exit;
			$trabajadoresPorJefe = $this->EvaluacionesTrabajadore->find("all", array(
				"fields"=>array( "Trabajadore.id", "EvaluacionesAnio.estado", "Trabajadore.nombre", "Trabajadore.apellido_paterno", "Trabajadore.fecha_ingreso", "Trabajadore.estado", "Cargo.nombre", "CargosFamilia.nombre", 
					"EvaluacionesTrabajadore.id", "EvaluacionesTrabajadore.fecha_inicio", "EvaluacionesTrabajadore.fecha_modificacion", "EvaluacionesTrabajadore.fecha_reunion", "EvaluacionesTrabajadore.lugar_reunion", 
					"EvaluacionesTrabajadore.hora_reunion", "EvaluacionesTrabajadore.evaluaciones_estado_id", "EvaluacionesEstado.id", "EvaluacionesEstado.nombre", "EvaluacionesTrabajadore.anio"),
				"joins"=> array( array("table" => "cargos_familias", "alias" => "CargosFamilia", "type" => "LEFT", "conditions" => array("Cargo.cargos_familia_id = CargosFamilia.id"))),				
				"conditions"=> array(
					"Trabajadore.id" => $IdTrabajadore,
					"Trabajadore.estado" => 'Activo',
				"EvaluacionesTrabajadore.evaluaciones_estado_id" => array(10, 11 ,12)
			),
				"order"=>array("Trabajadore.fecha_ingreso ASC")
			));
		...
		if(!empty($trabajadoresPorJefe)){
			foreach ($trabajadoresPorJefe as $trabajador) {
				$listaTrabajadores[] = array(
					...
					"fecha_reunion"		=> (isset($trabajador["EvaluacionesTrabajadore"]["fecha_reunion"]))? $trabajador["EvaluacionesTrabajadore"]["fecha_reunion"]:'-',
					"hora_reunion"			=> (isset($trabajador["EvaluacionesTrabajadore"]["hora_reunion"]))? $trabajador["EvaluacionesTrabajadore"]["hora_reunion"]:'-',	
					"anio"	=> $trabajador["EvaluacionesTrabajadore"]["anio"]
					);
			}
		}else{
			$listaTrabajadores = array();
...
/*
	public function listado_evaluaciones2(){
		...
		//$datosTrabajador["User"]["trabajadore_id"] = 403;
			...
			$trabajadoresPorJefe = $this->Trabajadore->find("all", array(
				...
				"conditions"=>array(  
					...
					"OR" => array(
						...
						array("EvaluacionesTrabajadore.evaluaciones_estado_id"=> 0)
						)
					),
				"order"=>array("Trabajadore.fecha_ingreso ASC"),
				"recursive"=>0
...
*/
	...
	public function listado_calibrador() {
		...
		/*if ($datosTrabajador["User"]["trabajadore_id"] == 403)
			$datosTrabajador["User"]["trabajadore_id"] = 484;
		*/		
		$evaluaciones = $this->EvaluacionesTrabajadore->find("first", array(
			"fields" => array("EvaluacionesTrabajadore.id"),
			"conditions"=>array("EvaluacionesTrabajadore.trabajadore_id"=> $datosTrabajador["User"]["trabajadore_id"],
		...
		$datosJefe = $this->Jefe->find("first", array(
			"fields"=>array("Jefe.id", "Trabajadore.id", "Trabajadore.nombre", "Trabajadore.apellido_paterno", "Trabajadore.cargo_id", "Trabajadore.foto", "Cargo.nombre", 
				"TrabajadoresCalibradore.id", "TrabajadoresCalibradore.nombre", "TrabajadoresCalibradore.apellido_paterno", "TrabajadoresCalibradore.cargo_id", "TrabajadoresCalibradore.foto", 
				"TrabajadoresCalibradore.jefe_id", "TrabajadoresCalibradore.email", "CargosCalibradore.nombre", "EvaluacionesTrabajadore.id" ),
			"joins" => array( array( "table" => "cargos", "alias" => "Cargo", "type" => "LEFT", "conditions" => array("Trabajadore.cargo_id = Cargo.id")),
				array( "table" => "jefes", "alias" => "Calibradore", "type" => "LEFT", "conditions" => array("Trabajadore.jefe_id = Calibradore.id")),
				array( "table" => "trabajadores", "alias" => "TrabajadoresCalibradore", "type" => "LEFT", "conditions" => array("Calibradore.trabajadore_id = TrabajadoresCalibradore.id")),
				array( "table" => "cargos", "alias" => "CargosCalibradore", "type" => "LEFT", "conditions" => array("TrabajadoresCalibradore.cargo_id = CargosCalibradore.id")),
			...
			"conditions"=>array( array("Jefe.trabajadore_id" => $datosTrabajador["User"]["trabajadore_id"]),
				/*"OR" => array( // restriccion cascada
					array("Trabajadore.id"=>array(110,111)),
					array("EvaluacionesAnio.estado" => 2)
					),*/
				),
			"recursive"=> 0
			));
		...
		$TrabajadoresCalibradore = $this->Trabajadore->find("all", array(
			...
			"conditions"=> array("EvaluacionesTrabajadore.validador_trabajadore_id" => $datosTrabajador["User"]["trabajadore_id"],	
				...
				//"Trabajadore.id" => $this->trabajadoresAEvaluar()
				"EvaluacionesTrabajadore.validador_trabajadore_id" => $datosJefe["Trabajadore"]["id"],
				//""
				),
			"order"=> array("EvaluacionesTrabajadore.fecha_modificacion ASC"),
			"recursive"=>-1
			));
			//pr($TrabajadoresCalibradore);
	...
	public function listado_desempeno(){
		...
		$anioEvaluado = $this->EvaluacionesAnio->find("first", array( 
			"fields"=>array("anio_evaluado", "inicio_ocd"), 
			"conditions"=>array("estado"=>2)			
			));
		$anioAEvaluar = isset($anioEvaluado["EvaluacionesAnio"]["anio_evaluado"])? $anioEvaluado["EvaluacionesAnio"]["anio_evaluado"]: null;	
		...
		if(empty($anioOCD)){
			$anioOCD["EvaluacionesAnio"]["anio_evaluado"] = $anioAEvaluar;
		}
		...
		/*if ($datosUser["User"]["trabajadore_id"] == 403){
			$datosUser["User"]["trabajadore_id"] = 269;
		}*/
			...
			foreach ($evaluaciones as $evaluacion) {
				$listEvaluaciones[] = array_merge( $evaluacion["EvaluacionesTrabajadore"],
					array("nombre_jefatura" 	=> $serv->capitalize(strtok($evaluacion["JefeTrabajadore"]["nombre"], " "). " " .$evaluacion["JefeTrabajadore"]["apellido_paterno"]),
						...
						"estado_actual"			=> ($evaluacion["EvaluacionesAnio"]["estado"]==2 && $evaluacion["EvaluacionesTrabajadore"]["evaluaciones_estado_id"]!=9 )? 1: 0,		//** 
						)
					);
			}
		}else{
		...
		$objetivos = $this->EvaluacionesTrabajadore->find("all", array(
			"fields" => array("EvaluacionesTrabajadore.id", "EvaluacionesObjetivo.id", "EvaluacionesObjetivo.descripcion_objetivo", "EvaluacionesObjetivo.porcentaje_ponderacion", "EvaluacionesObjetivo.indicador",
				...
				"EvaluacionesObjetivo.estado", "EvaluacionesTrabajadore.anio", "EvaluacionesObjetivo.evaluaciones_trabajadore_id", "EvaluacionesObjetivo.evaluaciones_objetivo_estado_id"),
			...
			"conditions"=>array(
				/*"OR"=>array(
					array(
						"AND"=>array(
							array("EvaluacionesTrabajadore.anio"=>$anioAEvaluar),
							array("EvaluacionesObjetivo.evaluaciones_objetivo_estado_id" => array(7))
						)
					),
					array(
						"AND"=>array(
							array("EvaluacionesTrabajadore.anio"=>$anioOCD["EvaluacionesAnio"]["anio_evaluado"]),
							array("EvaluacionesObjetivo.evaluaciones_objetivo_estado_id" => array(6,7))
						)
					)
				),		*/		
			...
			"order" => "EvaluacionesObjetivo.id DESC",
			"recursive"=>-1,
			"limit"=>3
			)
		);
		...
		if(!empty($listadoObjetivos))
			$listadoObjetivos = $serv->sort_array_multidim($listadoObjetivos,"objetivos_comentario_id ASC");		

		$listadoEvaluaciones = array("datosTrabajador"=>$trabajador,
			"listEvaluaciones"=>$listEvaluaciones,
			"anio_evaluar" => $anioAEvaluar,
			"anio_ocd" 	=> isset($objetivos[0]["EvaluacionesTrabajadore"]["anio"])? $objetivos[0]["EvaluacionesTrabajadore"]["anio"]: (isset($anioOCD["EvaluacionesAnio"]["anio_evaluado"]) ? $anioOCD["EvaluacionesAnio"]["anio_evaluado"] : $anioAEvaluar),
	...
	public function evaluaciones_final_json($anio=null){
		...
		$anioEvaluado = $this->EvaluacionesAnio->find("all", array( "conditions"=>array("estado >"=>0) ));		

		foreach ($anioEvaluado as $anios){
			if($anios["EvaluacionesAnio"]["estado"]==2 && ($anio==0 || !$anio) )
		...
		$listadoAnios = $serv->sort_array_multidim($listadoAnios, "anio_evaluado desc");
		$evaluacionesTrabajadores = $this->Trabajadore->find("all", array( 
			"fields" => array("EvaluacionesTrabajadore.id", "EvaluacionesTrabajadore.puntaje_ponderado", "EvaluacionesTrabajadore.puntaje_objetivos", "EvaluacionesTrabajadore.puntaje_competencias", 
				...
				"EvaluacionesTrabajadore.cargo_id", "EvaluacionesTrabajadore.gerencia_id", "EvaluacionesTrabajadore.niveles_logro_id", "EvaluacionesTrabajadore.acepta_trabajador", "EvaluacionesTrabajadore.modified",
		...
		foreach ($evaluacionesTrabajadores as $evaluacion) {
			...
			foreach($niveles as $nivel) 
				...
				$estado 			= isset($evaluacion["EvaluacionesTrabajadore"]["evaluaciones_estado_id"]) ? $evaluacion["EvaluacionesEstado"]["nombre"] : "Sin OCD";
				...
				$listadoFinal[] = array( 
					...
					"ultimo_cambio" => $evaluacion["EvaluacionesEstado"]["id"] > 0 ? substr($evaluacion["EvaluacionesTrabajadore"]["modified"],0,10) : '-'
					);
		}
	...
	public function evaluaciones_graficos_json(){
		...
		$listadoAnios = array();
		$anioActual = date("Y");				
		$anioEvaluado = $this->EvaluacionesAnio->find("all", array( "conditions"=>array("estado"=>array(2,3))));		
		
		foreach ($anioEvaluado as $anios){
			if($anios["EvaluacionesAnio"]["estado"]==2 && !$anio)
				$anio = $anios["EvaluacionesAnio"]["anio_evaluado"];
			if($anios["EvaluacionesAnio"]["anio_evaluado"]<$anioActual)
				$listadoAnios[] = $anios["EvaluacionesAnio"];
		}
		...
		$evaluacionesTrabajadores = $this->Trabajadore->find("all", array( 
			...
			"conditions" => array(
				"Trabajadore.id" => $this->trabajadoresAEvaluar(($anio+1).'-01-01'),
				"Trabajadore.id not" => array(501,110)
				),
			"joins" => array( array("table" => "evaluaciones_trabajadores", "alias" => "EvaluacionesTrabajadore", "type" => "LEFT", "conditions" => array("Trabajadore.id = EvaluacionesTrabajadore.trabajadore_id AND EvaluacionesTrabajadore.anio = '$anio'")),	
				array("table" => "trabajadores", "alias" => "Evaluadore", "type" => "LEFT", "conditions" => array("EvaluacionesTrabajadore.evaluador_trabajadore_id = Evaluadore.id")),	
				array("table" => "trabajadores", "alias" => "Jefatura", "type" => "LEFT", "conditions" => array("Jefe.trabajadore_id = Jefatura.id")),	
		...
		//pr($evaluacionesTrabajadores);exit;
	...
	public function grafico_cumplimiento($evaluaciones){
		...
		$cumplimiento = array(
			array( "nombre"=>"Colaboradores Evaluados " . "(" . $totalEvFinalizadas . ")",
				"valor"=> $porcentajeEvaluados
				),
			array( "nombre"=>"Colaboradores No Evaluados " . "(" .  ($totalEvaluaciones - $totalEvFinalizadas) . ")",
	...
	public function evaluaciones_detalle_final_json($anio=null){
		$this->layout = "ajax";
		$this->response->type('json');
		$this->loadModel("EvaluacionesAnio");
		$this->loadModel("Trabajadore");
		...
		$detalleCompetencias = $this->EvaluacionesTrabajadore->find("all",array(
			"fields" => array("EvaluacionesTrabajadore.id"),
			"recursive" => 0
			));		

		foreach ($detalleCompetencias as $competencias) {				
			$idEvaluacion = $competencias["EvaluacionesTrabajadore"]["id"];			
			$compHorizontal[$idEvaluacion] = array();				
			$i = 1;		
			foreach ($competencias["EvaluacionesCompetencia"] as $cEspecificas) {												
				$comp = array("competencia".$i => (isset($cEspecificas["puntaje_modificado"]))? $cEspecificas["puntaje_modificado"] : $cEspecificas["puntaje"]);
				$i++;
				$compHorizontal[$idEvaluacion] = array_merge($compHorizontal[$idEvaluacion], $comp);
			}			
			foreach ($competencias["EvaluacionesCompetenciasGenerale"] as $cGenerales) {				
				$comp = array("competencia".$i => (isset($cGenerales["puntaje_modificado"]))? $cGenerales["puntaje_modificado"] : $cGenerales["puntaje"]);
				$i++;
				$compHorizontal[$idEvaluacion] = array_merge($compHorizontal[$idEvaluacion], $comp);						
			}					
		}		
		...
		foreach ($evaluacionesTrabajadores as $evaluacion) {			
			...
			foreach($niveles as $nivel) 
				...
				$listadoFinal[] = array_merge( array("rut_trabajador" => implode("",explode(".",$evaluacion["Trabajadore"]["rut"]))),
					(array_key_exists($evaluacion["EvaluacionesTrabajadore"]["id"], $compHorizontal))? $compHorizontal[$evaluacion["EvaluacionesTrabajadore"]["id"]]: array()
					);
		}	
		usort($listadoFinal, function($a, $b) {
			$a = explode("-",$a['rut_trabajador']);
			$b = explode("-",$b['rut_trabajador']);
		    return $a[0] - $b[0];
		});
		$this->set("respuesta", $listadoFinal);
	}

	public function viewBotonHistoria(){}
		...
		if(!empty($evaluacionesTrabajadores))
		{
			...
			if(!empty($envioCorreo)){
				foreach ($envioCorreo as $datosCorreo) {
					...
					$Email->replyTo(array('sgi@cdf.cl' => 'SGI'));
					$Email->to($datosCorreo["email"]); 
					$Email->bcc('ddiaz@cdf.cl',"marriola@cdf.cl"); 
